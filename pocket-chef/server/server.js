...
app.use(session({
	secret: '2',//process.env.PASSPORT_SECRET
	resave: false,
...
app.post('/node_save_recipe', (req, res) => {
    if (req.isAuthenticated()) {
        //Add car to user's likes
        let likedRecipe = req.body;

        if (!likedRecipe.comments) {
            likedRecipe = {...likedRecipe, comments: []};
        }
        ...
        User.findOne({_id: req.user._id}, function(err, data) {
            if (err) {
                ...
                //If newly-liked car is already in User's liked cars, do not add again
                let savedRecipes = data.savedRecipes;
                for (let i in savedRecipes) {
                    //console.log(likedCarsArr[i].stock_num);
                    if (savedRecipes[i]._id === likedRecipe._id) {
                        //Car is already liked
